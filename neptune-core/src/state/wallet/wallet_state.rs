...
use super::address::pokolen_address;
...
impl WalletState {
    ...
    pub(crate) async fn try_new(
        ...
        let known_generation_keys = (0..rusty_wallet_database.get_forthegeneration_key_counter())
            .map(|idx| wallet_entropy.nth_forthegeneration_spending_key(idx).into())
            .collect_vec();
        ...
        let premine_keys = (0..NUM_PREMINE_KEYS)
            .map(|n| wallet_state.nth_spending_key(KeyType::Pokolen, n as u64))
            .collect_vec();
    ...
    /// I.e. to limit how many payments the wallet can send per block.
    ...
    pub fn get_known_spending_keys(
        ...
        match key_type {
            KeyType::Pokolen => Box::new(self.get_known_generation_spending_keys()),
    ...
    pub async fn next_unused_spending_key(&mut self, key_type: KeyType) -> SpendingKey {
        match key_type {
            KeyType::Pokolen => self.next_unused_forthegeneration_spending_key().await.into(),
    ...
    pub async fn bump_derivation_index(&mut self, key_type: KeyType, max_used_index: u64) {
        ...
        if current_counter < max_used_index + 1 {
            ...
            match key_type {
                KeyType::Pokolen => {
                    self.wallet_db
                        .set_forthegeneration_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                        let key = self.wallet_entropy.nth_forthegeneration_spending_key(idx).into();
    ...
    pub fn key_counter(&self, key_type: KeyType) -> u64 {
        match key_type {
            KeyType::Pokolen => self.wallet_db.get_forthegeneration_key_counter(),
    ...
    pub fn nth_spending_key(&self, key_type: KeyType, derivation_index: u64) -> SpendingKey {
        match key_type {
            KeyType::Pokolen => self
                .wallet_entropy
                .nth_forthegeneration_spending_key(derivation_index)
                .into(),
    ...
    /// Note that incrementing the counter modifies wallet state.  It is
    /// important to write to disk afterward to avoid possible funds loss.
    async fn next_unused_forthegeneration_spending_key(&mut self) -> pokolen_address::PokolenSpendingKey {
        let index = self.wallet_db.get_forthegeneration_key_counter();
        self.wallet_db.set_forthegeneration_key_counter(index.saturating_add(1)).await;
        let key = self.wallet_entropy.nth_forthegeneration_spending_key(index);
    ...
    pub(crate) fn get_future_generation_spending_keys(
        &self,
        num_future_keys: usize,
    ) -> impl Iterator<Item = (u64, pokolen_address::PokolenSpendingKey)> + use<'_> {
        let index = self.wallet_db.get_forthegeneration_key_counter();
        (index..index + (num_future_keys as u64))
            .map(|i| (i, self.wallet_entropy.nth_forthegeneration_spending_key(i)))
    }
...
pub(crate) mod tests {
    ...
    pub async fn outgoing_transaction(
        ...
        let an_address = PokolenReceivingAddress::derive_from_seed(rness.digests[0]);
        let tx_output =
    ...
    /// nonzero balance.
    ///
    /// Note that this function is probabilistic. Block is invalid, both w.r.t.
    /// PoW and proof.
    pub(crate) async fn bob_mines_one_block(
        network: Network,
    ) -> (Block, GlobalStateLock, PokolenSpendingKey) {
    ...
    async fn never_store_same_utxo_twice_different_blocks() {
        ...
        let random_address = PokolenReceivingAddress::derive_from_seed(rng.random());
    ...
    mod guesser_fee_utxos {
        ...
        async fn guesser_fee_scanner_finds_guesser_fee_iff_present() {
            ...
            let composer_key = wallet_state.wallet_entropy.nth_forthegeneration_spending_key(0);
            let genesis_block = Block::genesis(network);
    ...
    mod wallet_balance {
        ...
        async fn confirmed_and_unconfirmed_balance() -> Result<()> {
            ...
            let tx = {
                ...
                let outputs = vec![(
                    ReceivingAddress::from(PokolenReceivingAddress::derive_from_seed(
                        rng.random(),
                    )),
                    send_amt,
        ...
        async fn do_not_attempt_to_spend_utxos_already_spent_in_mempool_txs() {
            ...
            let (block, composer_expected_utxos) =
                make_mock_block_with_puts_and_guesser_preimage_and_guesser_fraction(
                    ...
                    vec![],
                    None,
                    alice_wallet.nth_forthegeneration_spending_key(0),
                    rng.random(),
    ...
    mod expected_utxos {
        ...
        async fn no_expected_utxos_on_custom_coinbase_distribution_and_offchain_notifications() {
            ...
            let wallet_state = mock_genesis_wallet_state(wallet, &cli_args).await;
            let an_address = PokolenReceivingAddress::derive_from_seed(Default::default());
        ...
        async fn abandoned_utxo_is_unsynced() {
            ...
            let (block_1a, composer_expected_utxos_1a) =
                make_mock_block_with_puts_and_guesser_preimage_and_guesser_fraction(
                    ...
                    vec![],
                    None,
                    alice_wallet.nth_forthegeneration_spending_key(14),
                    rng.random(),
                    (guesser_fraction, guesser_address.into()),
                    network,
                )
    ...
    pub(crate) mod scan_mode {
        ...
        async fn test_recovery_on_imported_wallet() {
            ...
            let alice_future_spending_key =
                alice_secret.nth_forthegeneration_spending_key(derivation_index);
            ...
            for (cli_args, should_catch_utxo) in [
                ...
                if should_catch_utxo {
                    ...
                    assert_eq!(
                        derivation_index + 1,
                        alice_wallet_state.wallet_db.get_forthegeneration_key_counter()
                    );
                    assert_eq!(
                        derivation_index + 1,
                        alice_wallet_state
                    ...
                    assert_eq!(1, alice_wallet_state.wallet_db.get_forthegeneration_key_counter());
                    assert_eq!(
                        1,
                        alice_wallet_state
                            .get_known_generation_spending_keys()
        ...
        async fn get_future_keys_do_not_modify_counters() {
            ...
            let generation_counter = wallet_state.wallet_db.get_forthegeneration_key_counter();
            ...
            assert_eq!(
                generation_counter,
                wallet_state.wallet_db.get_forthegeneration_key_counter(),
            );
            assert_eq!(
                symmetric_counter,
            ...
            let future_generation_keys = future_generation_relative_indices
                ...
                .map(|(relative_index, absolute_index)| {
                    (
                        ...
                        absolute_index,
                        SpendingKey::from(
                            wallet_secret.nth_forthegeneration_spending_key(absolute_index),
                        ),
                    )
                })
                .collect_vec();
    ...
    pub(crate) mod fee_notifications {
        ...
        async fn wallet_recovers_upgrader_fee_utxos_given_notification_policy(
            ...
            let destination_address =
                PokolenSpendingKey::derive_from_seed(rng.random()).to_address();
