...
/// deterministic announcements, since these are needed to be able to reuse proofs for tests. These values are used in the encryption
/// step.
///
...
pub fn deterministically_derive_seed_and_nonce(
    ...
    let combined = Tip5::hash_pair(payload.sender_randomness, payload.utxo.lock_script_hash().0);
