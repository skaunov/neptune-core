...
//! ### Naming
//!
//! These was called `Generation...` keys because they are quantum-secure and it is
//! believed/hoped that the cryptography should be unbreakable for at least a generation and hopefully many generations. 
//! If correct, it would be safe to put funds in a paper or metal wallet and ignore them for decades, perhaps
//! until they are transferred to the original owner's children or grand-children.
//! 
//! As the name is so much confusing with the verb and I could not find a disabmiguate wording, I just substitute it with `Pokolenie...`. 
//! I guess a proper solution would be something based on "quantum-secure" but I'm tired of searching the words and want my local fix to diverge minimally.
...
pub(super) const POKOLEN_FLAG_U8: u8 = 79;
pub const POKOLEN_FLAG: BFieldElement = BFieldElement::new(POKOLEN_FLAG_U8 as u64);

pub const HRP_PREFIX: &str = "nolga";

/// note: we `serde(skip)` fields that can be computed from the seed in order to keep 
/// the serialized (including bech32m) representation small
#[derive(Clone, Debug, Copy, PartialEq, Eq, Serialize)]
pub struct PokolenSpendingKey {
    seed: Digest,
...
impl<'de> serde::de::Deserialize<'de> for PokolenSpendingKey {
    /* todo: is there a more succinct way to `impl` this `fn` that works for both sequential 
    and map visitor access patterns? */
    //
        ...
        D: serde::de::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for FieldVisitor {
            type Value = PokolenSpendingKey;

            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("struct PokolenSpendingKey")
            }
                ...
                V: serde::de::SeqAccess<'de>,
            {
                ...
                Ok(Self::Value::derive_from_seed(seed))
            }
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                Ok(Self::Value::derive_from_seed(seed_digest))
            }
        }

        const FIELDS: &[&str] = &["seed"];
        deserializer.deserialize_struct("PokolenSpendingKey", FIELDS, FieldVisitor)
    }
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub struct PokolenReceivingAddress {
    receiver_identifier: BFieldElement,
...
impl PokolenSpendingKey {
    pub fn to_address(&self) -> PokolenReceivingAddress {
        PokolenReceivingAddress {
            receiver_identifier: self.receiver_identifier(),
            encryption_key: lattice::kem::keygen(common::shake256::<32>(&bincode::serialize(&self.seed).unwrap())).1,
            receiver_postimage: self.receiver_preimage.hash(),
    ...
    pub fn derive_from_seed(seed: Digest) -> Self {
        ...
        let decoded_address = PokolenReceivingAddress::from_bech32m(&encoded_address, network).unwrap();
        assert_eq!(
...
impl PokolenReceivingAddress {
    pub fn from_spending_key(spending_key: &PokolenSpendingKey) -> Self {
        ...
        Self {
            receiver_identifier,
            encryption_key: lattice::kem::keygen(randomness).1,
    ...
    pub fn derive_from_seed(seed: Digest) -> Self {
        let spending_key = PokolenSpendingKey::derive_from_seed(seed);
    ...
    pub fn from_bech32m(encoded: &str, network: Network) -> Result<Self> {
        ...
        bincode::deserialize(&Vec::<u8>::from_base32(&data)?)
    ...
    ///   8 end of address.
    /// ```
    /// It would be nice to standardize on a single prefix-len. 6 chars seems a bit much. 
    /// Maybe we could shorten generation prefix to 4 somehow, e.g. 
    /// ngkm --> neptune-generation-key-mainnet
    ///
    /// # Deprecated.
    /// Use [super::receiving_address::ReceivingAddress::to_display_bech32m_abbreviated] instead.
...
mod tests {
    ...
    mod generation_spending_key {
        ...
        // there is a custom impl Deserialization for PokolenReceivingAddress
        //
        ...
        mod serialization {
            ...
            pub fn roundtrip_bincode() {
                let spending_key = PokolenSpendingKey::derive_from_seed(rand::random());

                let s = bincode::serialize(&spending_key).unwrap();
                let deserialized_key: PokolenSpendingKey = bincode::deserialize(&s).unwrap();
            ...
            pub fn roundtrip_json() {
                ...
                let deserialized_key: PokolenSpendingKey = serde_json::from_str(&s).unwrap();
    ...
    fn can_parse_valid_address() {
        ...
        // no crash
        let _ = PokolenReceivingAddress::from_bech32m(bech32m_string, network).unwrap();
    }
    ...
    fn no_crash_in_bech32_decoding() {
        ...
        for str in [short_prefix, long_prefix] {
            assert!(
                PokolenReceivingAddress::from_bech32m(&str, network).is_err(),
        ...
        for i in 0..10 {
            let as_ = "a".repeat(i);
            assert!(
                PokolenReceivingAddress::from_bech32m(&as_, network).is_err(),
                "Invalid bech32 encoding must lead to error 1"
            );
            assert!(
                PokolenReceivingAddress::from_bech32m(&format!("{HRP_PREFIX}1{as_}"), network)
                    .is_err(),
                "Invalid bech32 encoding must lead to error 2"
            );

            assert!(
                PokolenReceivingAddress::from_bech32m(&format!("{SHORT_PREFIX}1{as_}"), network)
                    .is_err(),
                "Invalid bech32 encoding must lead to error 4"
            );
            assert!(
                PokolenReceivingAddress::from_bech32m(&format!("1{as_}"), network).is_err(),
    ...
    fn encryption_is_deterministic() {
        ...
        let result = super::PokolenSpendingKey::derive_from_seed(Digest::default())
            .to_address()
            .encrypt(&payload);
