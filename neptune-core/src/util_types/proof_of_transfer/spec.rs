    ...
    for super::ProofOfTransfer
{
    ...
    > There is a discrepancy between the spec and the tasm code. The first thing the spec reads is the release date. 
    The first thing the tasm code reads is the receiver digest. */
    fn source(&self) {
        ...
        tasmlib_io_write_to_stdout___digest(proof_of_transfer_witness.utxo.lock_script_hash().0);
