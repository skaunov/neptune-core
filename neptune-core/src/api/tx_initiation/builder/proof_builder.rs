...
impl<'a> ProofBuilder<'a> {
    ...
    /// generate the proof
    ///
    /// if the network uses mock proofs (eg Network::RegTest), this will return immediately with a mock [Proof]
    ///
    ...
    /// CPU cores and RAM.
    ///
    /// Given the serialized nature of the job-queue, it is possible or even likely that other jobs may precede this one.
    ///
    /// One can query the job_queue to determine how many jobs are in the queue.
    ///
    /// # RegTest mode.
    /// mock proofs are used on the regtest network (only) because they can be generated instantly
    ///
    /// # External Process.
    ///
    /// Proofs are generated in the Triton VM. The proof generation occurs in a separate executable, `triton-vm-prover`, which is spawned by the
    /// job-queue for each proving job. Only one `triton-vm-prover` process should be executing at a time for a given `neptune-core` instance.
    ///
    /// If the external process is killed for any reason, the proof-generation job will fail and this method will return an error.
    ///
    /// # Cancellation.
    ///
